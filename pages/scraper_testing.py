...
with tab3:
    st.markdown("### Batch Platform Testing")
    ...
    with col1:
        batch_query = st.text_input(
            "Search Query (for all platforms):",
            placeholder="e.g., iPhone 15, laptop, shoes",
            key="batch_search_query"
        )
    ...
    with col2:
        batch_limit = st.slider(
            ...
            max_value=10,
            value=3,
            key="batch_limit_slider"
        )
    
    # Batch options
    with st.expander("Batch Test Options", expanded=False):
        batch_mode = st.selectbox(
            "Test Mode:",
            options=["standalone", "offline"],
            format_func=lambda x: {"standalone": "Live Test", "offline": "Cached"}[x],
            key="batch_mode_selector"
        )
        
        include_platforms = st.multiselect(
            "Include Platforms:",
            options=SUPPORTED_PLATFORMS,
            default=SUPPORTED_PLATFORMS,
            format_func=format_platform_name,
            key="batch_platform_selector"
        )
    ...
    with button_col1:
        if st.button(" Test All Platforms", type="primary", use_container_width=True, disabled=st.session_state["test_in_progress"]):
            if not batch_query:
                st.error("Please enter a search query for batch testing")
            else:
                st.session_state["test_in_progress"] = True
                ...
                # Execute batch test
                with st.spinner("Testing all platforms... This may take a while..."):
                    try:
                        result = api.test_all_scrapers(token, batch_test_data)
                        st.session_state["batch_test_results"] = result
                        
                        # Show summary
                        summary = result.get("summary", {})
                        st.success(f"Batch test completed!")
                        st.info(f"Success Rate: {summary.get('success_rate', 0):.1f}% | "
                               f"Total Products: {summary.get('total_products_found', 0)}")
                    except ApiError as e:
    ...
    with button_col2:
        if st.button(" Quick Fix All", use_container_width=True):
            st.info("Auto-fix for all platforms coming soon...")
    ...
    if "batch_test_results" in st.session_state:
        ...
        batch_result = st.session_state["batch_test_results"]
        
        # Summary metrics
        summary_col1, summary_col2, summary_col3, summary_col4 = st.columns(4)
        with summary_col1:
            st.metric("Total Platforms", batch_result.get("total_platforms", 0))
        with summary_col2:
            st.metric("Successful", batch_result.get("successful_tests", 0))
        with summary_col3:
            st.metric("Failed", batch_result.get("failed_tests", 0))
        with summary_col4:
            summary = batch_result.get("summary", {})
            duration = summary.get("total_duration_ms", 0)
            st.metric("Total Duration", format_duration(duration))
        
        # Platform results table
        platform_results = batch_result.get("platform_results", [])
        if platform_results:
            ...
            for result in platform_results:
                ...
                df_data.append({
                    "Platform": format_platform_name(result["platform"]),
                    "Status": "" if result.get("success") else "",
                    "Products Found": test_results.get("products_found", 0),
                    "Duration": format_duration(test_results.get("test_duration_ms", 0)),
                    "Error": test_results.get("error_message", "")[:50] + "..." if test_results.get("error_message") else ""
                })
            ...
            selected_result_platform = st.selectbox(
                "View details for:",
                options=[r["platform"] for r in platform_results],
                format_func=format_platform_name,
                key="batch_result_selector"
            )
            
            if selected_result_platform:
                platform_result = next((r for r in platform_results if r["platform"] == selected_result_platform), None)
                if platform_result:
                    with st.container(border=True):
                        st.write(f"**{format_platform_name(platform_result['platform'])} Results**")
                        
                        if platform_result.get("success"):
                            test_results = platform_result.get("results", {})
                            st.success(f"Success - Found {test_results.get('products_found', 0)} products")
                            st.caption(f"Duration: {format_duration(test_results.get('test_duration_ms', 0))}")
                            
                            # Sample products
                            sample_products = test_results.get("sample_products", [])
                            if sample_products:
                                st.write("**Sample Products:**")
                                for product in sample_products[:3]:
                                    st.write(f"- {product.get('title', 'N/A')} ({product.get('brand', 'N/A')})")
                        else:
                            st.error(f"Failed: {platform_result.get('message', 'Unknown error')}")
                            test_results = platform_result.get("results", {})
                            if test_results.get("error_message"):
                                st.code(test_results["error_message"])
